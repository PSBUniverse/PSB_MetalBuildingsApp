...
function getSettingsCacheKey(actorUserId) {
  return createCacheKey("admin-users", actorUserId);
}
...
export default function SettingsPage() {
    ...
    async function loadSettings() {
      ...
      try {
        const cachedResult = await getOrFetchCached({
          key: getSettingsCacheKey(currentUser.userId),
  ...
  async function sendPatch(body) {
    ...
    if (currentUser?.userId) {
      invalidateCacheKey(getSettingsCacheKey(currentUser.userId), {
        namespace: SETTINGS_CACHE_NAMESPACE,
      });
    }
